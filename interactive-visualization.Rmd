...
# deploy app to Shiny apps account 
...
server <- function(input, output) { 
  ...
  output$hist <- renderPlot({ 
    ...
    ggplot(data = input$num, aes(x = "num", y = rnorm("num")) + geom_histogram(stat = "identity"))
...
server <- function(input, output) {
  output$hist <- renderPlot(ggplot(data = as.data.frame(rnorm(input$num)), aes(x = input$num)) + geom_histogram()) } 
